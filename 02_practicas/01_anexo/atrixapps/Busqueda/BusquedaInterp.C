...
void interpolation_search(int [],int,int); //function prototype
...
void main(void)
{
	...
	scanf("%d", &x);
	bubble_sort(arr,n); //call by value
	interpolation_search(arr,n,x);
	getch();
}
...
void interpolation_search(int arr[],int n,int x)
{
	int left,right,mid;
	left=0;
	right= n-1;
